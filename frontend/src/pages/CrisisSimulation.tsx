  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6">
        <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">90-Day Risk Score Projection</h3>
        <p className="text-xs text-gray-600 dark:text-gray-400 mb-4">Monte Carlo simulation showing baseline, stressed, and severe scenarios for {data.bank.name}</p>
          ...
          <div key={i} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
            <div className="p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <h4 className="text-base font-semibold text-gray-900 dark:text-white">{scenario.name}</h4>
                    {impactBadge(scenario.impact)}
                  </div>
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    <div>
                      <p className="text-xs text-gray-500">Probability</p>
                      <p className="text-sm font-medium text-gray-700 dark:text-gray-300">{(scenario.probability * 100).toFixed(0)}%</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500">Recovery Timeline</p>
                      <p className="text-sm font-medium text-gray-700 dark:text-gray-300">{scenario.recovery_days} days</p>
                    </div>
                    <div>
              ...
              <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 border border-gray-300 dark:border-gray-700">
                <p className="text-xs text-gray-700 dark:text-gray-300 leading-relaxed whitespace-pre-line">{scenario.description}</p>
              </div>
            </div>
          </div>
        ))}
