    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Reputation Risk Assessment</h3>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="text-right ml-2">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">{report.report_date}</p>
            </div>
          </div>
        </div>
        <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4">
          <h4 className="text-xs font-medium text-gray-600 dark:text-gray-400 uppercase tracking-wider mb-2">Executive Summary</h4>
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400">Peer Group Risk Snapshot</h3>
        ...
        <div className="flex flex-wrap justify-center gap-6">
          {report.banks.map((b) => (
            <div key={b.bank.id} className="text-center">
              ...
              <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">{b.bank.name}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Key findings */}
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6">
          <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-4">Key Findings</h3>
        ...
        {/* Recommendations */}
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6">
          <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-4">Recommendations</h3>
          <div className="space-y-3">
            {report.recommendations.map((rec, i) => (
              <div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-gray-50 dark:bg-gray-800/30">
                {priorityBadge(rec.priority)}
                <p className="text-sm text-gray-700 dark:text-gray-300 flex-1">{rec.action}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
        <div className="p-4 border-b border-gray-200 dark:border-gray-800">
          <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400">Detailed Risk Scores</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {report.banks.map((b) => (
                <tr key={b.bank.id} className="border-b border-gray-200 dark:border-gray-800/50">
                  ...
                  <td className="p-3 text-right text-gray-600 dark:text-gray-400">{b.media_sentiment_score}</td>
                  <td className="p-3 text-right text-gray-600 dark:text-gray-400">{b.complaint_score}</td>
                  <td className="p-3 text-right text-gray-600 dark:text-gray-400">{b.market_score}</td>
                  <td className="p-3 text-gray-600 dark:text-gray-400 text-xs">{b.top_drivers[0]?.name} ({Math.round(b.top_drivers[0]?.score)})</td>
                </tr>
              ))}
            </tbody>
          </table>
